...
Locked before any model is trained, so that every baseline and every ablation
variant is scored by identical code.
...
* NaNs in either array are dropped pairwise. An all-NaN input returns NaN
  rather than raising, so a single empty stratum cannot abort a results sweep.
* Daytime masking is the caller's responsibility via :func:`daytime_mask`.
  Metrics do not silently filter, because a metric that quietly changes its own
  sample set is not comparable across models.
"""
...
def daytime_mask(clearsky_ghi, floor: float = DAYTIME_CLEARSKY_FLOOR) -> np.ndarray:
    """Boolean mask selecting daytime samples.

    Night-time GHI is identically zero and trivially predictable. Including it
    shrinks MAE, inflates R^2 toward 1, and makes MAPE undefined. Every headline
    metric in this project is computed on daytime samples only.

    Uses clear-sky GHI rather than measured GHI so the mask is a property of
    solar geometry alone and is therefore identical for every model compared.
    """
...
def mbe(y_true, y_pred) -> float:
    ...
    Reported per monsoon phase to expose systematic seasonal bias, which a
    symmetric metric such as RMSE cannot reveal.
    """
    t, p = _clean(y_true, y_pred)
...
def mape(y_true, y_pred, floor: float = MAPE_GHI_FLOOR) -> float:
    ...
    MAPE diverges as the denominator approaches zero, which for irradiance
    happens twice daily. The floor makes it computable, but it also makes the
    sample set differ from the other metrics — so prefer :func:`nmae` and
    :func:`nrmse` when reporting, and treat MAPE as a familiarity metric only.
    """
    t, p = _clean(y_true, y_pred)
    ok = t > floor
    if not ok.any():
...
def r2(y_true, y_pred) -> float:
    ...
    Note that R^2 on daytime irradiance is dominated by the diurnal cycle and
    will look high for almost any model. It is reported for convention, not as
    evidence of skill — Forecast Skill is the metric that matters.
    """
    t, p = _clean(y_true, y_pred)
    if t.size < 2:
        return float("nan")
...
def forecast_skill(y_true, y_pred, y_reference) -> float:
    ...
    1.0 is a perfect forecast, 0.0 matches the reference, negative is worse
    than the reference.

    The reference must be **smart persistence**, not naive persistence, for the
    headline number. Naive persistence is a weak reference in a strongly
    diurnal signal and inflates FS for every model; see
...
def picp(y_true, lower, upper) -> float:
    ...
    For a nominal P10/P90 interval a calibrated model returns ~0.80. Report
    this separately for monsoon-transition and stable windows: calibration
    characteristically degrades under regime shift, and an aggregate figure
...
def pinaw(y_true, lower, upper) -> float:
    ...
    PICP alone is trivially gamed by predicting an enormous interval. PINAW is
    the cost side of that trade and must always be reported alongside it.
    Normalised by the observed range so it is comparable across sites.
    """
    t = np.asarray(y_true, dtype=float).ravel()
    lo = np.asarray(lower, dtype=float).ravel()
    hi = np.asarray(upper, dtype=float).ravel()
...
def reliability_curve(y_true, y_pred_quantiles, quantiles):
    ...
    (nominal, empirical) : two 1-D arrays.
        A perfectly calibrated model lies on the diagonal. Points below the
        diagonal mean the intervals are too narrow — the model is overconfident.
    """
    t = np.asarray(y_true, dtype=float).ravel()
    ...
    if q_pred.shape[0] != t.shape[0]:
        raise ValueError("y_true and y_pred_quantiles disagree on sample count")
...
def point_metrics(y_true, y_pred, y_reference=None) -> dict[str, float]:
    ...
    ``y_reference`` should be the smart-persistence forecast; when omitted, the
    forecast-skill entry is NaN rather than absent, so that results frames keep
    a stable column set across strata.
    """
    out = {
