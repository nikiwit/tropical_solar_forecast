"""
Explore MetMalaysia API - find location IDs for KL, Penang, KK and
available datatypes. Token: set METMALAYSIA_TOKEN in .env Usage: python
...
# Exact substrings to match (no short abbreviations that cause false
# positives)
TARGETS = [
    "kuala lumpur",
    "pulau pinang",
...
def find_my_locations():
    ...
    matched = [
        t
        for t in towns
        if any(k in t.get("name", "").lower() for k in TARGETS)
    ]
    print(f"  Matched:")
    for t in matched:
...
def test_observation(location_id, location_name):
    ...
    if results:
        for row in results[:10]:
            print(
                f"  {row.get('date','')[:16]}  {row.get('datatype',''):12s}  {row.get('value','')}  {row.get('attributes',{}).get('unit','')}"
            )
    else:
...
def sample_forecast(location_id, location_name):
    ...
    for row in r.json().get("results", []):
        print(
            f"  {row['datatype']:10s}  {row['value']}  {row['attributes'].get('unit','')}"
        )
...
if __name__ == "__main__":
    ...
    if matched:
        kl = next(
            (t for t in matched if "lumpur" in t["name"].lower()), matched[0]
        )
